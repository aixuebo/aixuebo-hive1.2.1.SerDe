 ...
 * 1.该类有所有hive支持的typename、java原始class、java包装对象、hadoop序列化对象的对应关系,即定义了PrimitiveTypeEntry
 * 2.该类可以将任意一个对象实例和对象类型,转成成任意hive支持的对象
 */
public final class PrimitiveObjectInspectorUtils {
   ...
   * true表示参数class是java的基础类型和java对象
   */
   ...
   * 通过java原始或者java对象,获取对应的typeName
   */
   ...
   * 通过java原始对象或者java对象,返回PrimitiveTypeEntry对象
   */
  ...
  public static boolean comparePrimitiveObjectsWithConversion(Object o1,
      PrimitiveObjectInspector oi1, Object o2, PrimitiveObjectInspector oi2) {
    if (o1 == null || o2 == null) {//有任意一个是null,则返回false
      return false;
    }

    if (oi1.getPrimitiveCategory() == oi2.getPrimitiveCategory()) {//类型相同,则进行值比较
