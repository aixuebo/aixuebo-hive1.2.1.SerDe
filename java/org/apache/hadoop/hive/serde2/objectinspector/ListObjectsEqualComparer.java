...
public class ListObjectsEqualComparer {
  enum CompareType {
    ...
    COMPARE_TEXT,//Text比较
    COMPARE_INT, COMPARE_LONG, COMPARE_BYTE,COMPARE_BOOL,//int、long、byte、boolean比较
    OTHER //其他类型比较
  }

  class FieldComparer {
    protected ObjectInspector oi0, oi1;//两个list中元素的类型
    protected ObjectInspector compareOI;
    protected CompareType compareType;//比较类型
    protected Converter converter0, converter1;

    //根据list元素的类型不同,分别强转成以下变量
    ...
    public boolean areEqual(Object o0, Object o1) {
      ...
      case COMPARE_TEXT:
        return (soi0.getPrimitiveWritableObject(o0).equals(
            soi1.getPrimitiveWritableObject(o1)));//转换成Text序列化对象进行比较
      case COMPARE_INT:
        return (ioi0.get(o0) == ioi1.get(o1));//对java原始类型进行比较
      case COMPARE_LONG:
      ...
      case COMPARE_STRING:
        return (soi0.getPrimitiveJavaObject(o0).equals(
            soi1.getPrimitiveJavaObject(o1)));//对比String类型进行比较
      default:
        return (ObjectInspectorUtils.compare(
            o0, oi0, o1, oi1) == 0);//其他比较按照字节数组进行比较
      }
    }
  }
  ...
  public ListObjectsEqualComparer(ObjectInspector[] oi0, ObjectInspector[] oi1) {
    if (oi0.length != oi1.length) {//长度必须相同
    ...
    for (int i = 0; i < oi0.length; i++) {//比较每一个index位置的元素是否相同
