 ...
 * 不同类型的对象进行转换
 * 1.参见PrimitiveObjectInspectorConverterm,针对原始基本类型进行转换操作
 */
public final class ObjectInspectorConverters {

  /**
   * A converter which will convert objects with one ObjectInspector to another.
   * 类型转换接口,将输入对象实例input,转换成输出对象
   *
   * 详细参见PrimitiveObjectInspectorConverter类实现
   */
  ...
  public static class IdentityConverter implements Converter {
    ...
    /**
     * 根据输出类型,获取要转换的Converter类,该类就可以对输入进行转换成输出了
     * @param inputOI 输入类型
     * @param outputOI 输出类型
     */
